...
func (r *Repository) AddContactToGroup(
	groupID uuid.UUID,
...
func (r *Repository) checkExistContactInGroup(
	ctx context.Context,
	tx pgx.Tx,
	groupID uuid.UUID,
	contactIDs ...uuid.UUID,
) (listExist []uuid.UUID, mapExist map[uuid.UUID]bool, err error) {
	...
	query, args, err := r.genSQL.
		...
		Where(squirrel.Eq{"contact_id": contactIDs, "group_id": groupID}).
		ToSql()
	if err != nil {
		return nil, nil, err
	}
