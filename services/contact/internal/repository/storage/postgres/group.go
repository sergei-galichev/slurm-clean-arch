...
func (r *Repository) UpdateGroup(
	...
	query, args, err := r.genSQL.Update("slurm.group").
		Set("name", groupForUpdate.Name()).
		Set("description", groupForUpdate.Description()).
...
func (r *Repository) clearGroupTx(ctx context.Context, tx pgx.Tx, groupID uuid.UUID) error {
	query, args, err := r.genSQL.Delete("slurm.contact_in_group").
		Where(
			squirrel.Eq{
				"group_id": groupID,
			},
...
func (r *Repository) oneGroupTx(ctx context.Context, tx pgx.Tx, ID uuid.UUID) (*group.Group, error) {
	var builder = r.genSQL.Select(
		"id",
		"name",
		"description",
...
func (r *Repository) CountGroup() (uint64, error) {
	...
	var builder = r.genSQL.Select("COUNT(id)").From("slurm.group")
	builder = builder.Where(squirrel.Eq{"is_archived": false})
...
func (r *Repository) updateGroupsContactCountByFilters(
	ctx context.Context,
	tx pgx.Tx,
	ID uuid.UUID,
) error {
